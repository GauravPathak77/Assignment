...
mongoose.connect("mongodb://127.0.0.1/Sample_data", {
...
app.get("/api/data", (req, res) => {
  ...
  .exec()
  .then((data) => {
  const filteredData1 = data.filter((Data) => Number(Data.income.slice(1)) < 5 && (Data.car === "BMW" ||Data.car === "Mercedes-Benz"));
  const filteredData2 = data.filter((Data) => Data.gender === "Male" && Data.phone_price > 10000);
  const filteredData3 = data.filter((Data) => Data.last_name.charAt(0) === "M" && Data.quote.length > 15 && Data.email.includes(Data.last_name.toLowerCase()));
  const filteredData4 = data.filter((Data) => (Data.car === "BMW" || Data.car === "Mercedes-Benz" || Data.car === "Audi") && !/\d/.test(Data.email));
  const userData = {};
  for(let user of data){
    if(userData[user.city]){
      ...
      userData[user.city] = { count: 1, income: Number(user.income.slice(1)) };
    }
  }
